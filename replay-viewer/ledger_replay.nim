## Ledger static replay viewer, wasm side.
##
## JS hands the raw replay bytes to led_load_replay; this module parses them
...
proc ledLoadReplay(data: ptr uint8, length: cint): cint
    {.exportc: "led_load_replay", cdecl.} =
  try:
    lastError = ""
    let replay = parseJson(bytesFromPointer(data, int(length)))
    var config = defaultGameConfig()
    config.rounds = replay["config"]{"rounds"}.getInt(14)
    ...
    payload = $ %*{
      "type": "replay",
      "protocol": replay{"protocol"}.getStr("ledger.replay.v1"),
      "names": replay["names"],
...
proc ledPayloadPointer(): ptr uint8 {.exportc: "led_payload_ptr", cdecl.} =
  if payload.len == 0:
    nil
  else:
    cast[ptr uint8](payload[0].addr)

proc ledPayloadLength(): cint {.exportc: "led_payload_len", cdecl.} =
  cint(payload.len)

proc ledErrorPointer(): ptr uint8 {.exportc: "led_error_ptr", cdecl.} =
  if lastError.len == 0:
    nil
  else:
    cast[ptr uint8](lastError[0].addr)

proc ledErrorLength(): cint {.exportc: "led_error_len", cdecl.} =
  cint(lastError.len)
