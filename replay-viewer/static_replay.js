// Ledger static replay shell: fetches the replay named by ?replay=<url>,
...
(function () {
  ...
  function start(module, bytes) {
    ...
    var ok = module._led_load_replay(ptr, bytes.length);
    module._free(ptr);
    if (!ok) {
      fail(readString(module, module._led_error_ptr(),
        module._led_error_len()) || "wasm rejected the replay");
      return;
    }
    var payload = JSON.parse(
      readString(module, module._led_payload_ptr(),
        module._led_payload_len())
    );
    var loading = document.getElementById("loading");
    if (loading) loading.style.display = "none";
    document.documentElement.removeAttribute("data-replay-error");
    LedgerRenderer.attachReplay({
  ...
  function load() {
    ...
    caption(attempt > 1 ? "RETRYING REPLAY… (attempt " + attempt + ")" :
      "LOADING REPLAY…");
    if (!modulePromise) {
      modulePromise = LedgerReplayModule().catch(function (error) {
