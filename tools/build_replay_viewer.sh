...
test -s "${dist}/ledger_replay.wasm"
test -s "${dist}/ledger_replay.js"

rm -rf "${output_dir}"
# `coworld build` pre-creates the bundle's parent; a fresh CI checkout does
# not, and a hook that resolves its output path before the parent exists
# fails on the runner and nowhere else (ecos, 2026-08-23).
mkdir -p "$(dirname "${output_dir}")"
mkdir -p "${output_dir}/assets"
cp "${dist}/ledger_replay.js" "${dist}/ledger_replay.wasm" "${output_dir}/"
...
echo "ledger replay viewer bundle: ${output_dir}"
